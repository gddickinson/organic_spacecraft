...
# The picture itself lives next door (`ui/zone_canvas.py`), split out at
# five hundred lines; this file is the panel, the odds and the buttons.
from .zone_canvas import CELL, MARK, ZoneMap, feature_mark
...
class ExpeditionView(View):
    ...
    def _status(self, exp) -> Panel:
        ...
        if way["home"]:
            ...
            if way["pinned"]:
                p.add(note(way["why"]))
        # Where on the world they are (`sim/worldmap`), when they were put
        # down off the surface map rather than by the old door.
        cell = tuple(getattr(exp, "cell", (-1, -1)) or (-1, -1))
        if cell[0] >= 0:
            from ..sim import worldmap as worldmap_sim
            from ..sim import worldsites as worldsites_sim
            body = next((b for b in self.game.system.bodies
                         if b.id == exp.body_id), None)
            if body is not None:
                spot = worldmap_sim.of(self.game, body).at(*cell)
                standing = worldsites_sim.at(self.game, body, *cell)
                p.add_row("Where on the world",
                          f"{spot.lat:+.0f}°, {spot.lon:+.0f}°"
                          + (f" — {standing.name}" if standing else ""))
