...
def run(suite: Suite) -> None:
    ...
    @check("a world close enough to show its facets is cut finely enough to hide them")
    def _():
        # Flat shading gives each face one colour, so a sphere reads as the
        # polyhedron it is: at 22 rings by 30 segments you could count the quads
        # across the terminator of anything filling the window.
        #
        # **Four attempts to smooth the shading instead went in the bin.** A
        # `QLinearGradient` per face is constant perpendicular to its own axis
        # where real Gouraud varies, and that error alternates with a quad's
        # orientation — so corner-to-corner, along the projected light, and
        # ordered by latitude all put a *checkerboard* on the sphere. It was not
        # the rim term either; forcing that to zero left the pattern untouched.
        # Geometry is what worked, so this holds the two things that make it pay:
        # the fine mesh really is finer, and it is spent only where it shows.
        coarse_faces = len(worlds3d.mesh_for("rocky")[1])
        fine_faces = len(worlds3d.mesh_for("rocky", fine=True)[1])
        assert fine_faces > coarse_faces * 2.5, (
            f"the fine mesh is {fine_faces} faces against {coarse_faces} — not "
            "enough finer to be worth a second table")
        for kind in worlds3d.WORLD_PAINTS:
            for fine in (False, True):
                verts, faces = worlds3d.mesh_for(kind, fine=fine)
                assert verts and faces, (kind, fine)
            assert (len(worlds3d.mesh_for(kind, True)[1])
                    > len(worlds3d.mesh_for(kind)[1])), kind

        # And the viewport spends it by apparent size, not by kind: a world nine
        # pixels across looks the same either way and must not cost four times
        # as much.
        from ..ui.viewport import FINE_ABOVE
        assert 20 < FINE_ABOVE < 400, FINE_ABOVE
        big = worlds3d.mesh_for("rocky", fine=FINE_ABOVE + 1 >= FINE_ABOVE)
        small = worlds3d.mesh_for("rocky", fine=FINE_ABOVE - 1 >= FINE_ABOVE)
        assert len(big[1]) > len(small[1]), (
            "the threshold picks the same mesh either side of itself")

        # The rings and segments are a real trade and the pair is chosen on it:
        # rings sample the colour, which runs with latitude, and segments round
        # the silhouette. Neither may collapse.
        rings, segments = worlds3d.FINE
        assert rings > worlds3d.COARSE[0] and segments > worlds3d.COARSE[1], (
            f"fine is {worlds3d.FINE} against coarse {worlds3d.COARSE}")
        assert 1.0 < rings / segments < 2.5, (
            f"{rings} rings to {segments} segments is lopsided: all rings bands "
            "the colour, all segments stripes the limb")
        return (f"{coarse_faces} faces coarse, {fine_faces} fine at "
                f"{rings}x{segments}, spent above {FINE_ABOVE:.0f} px")
