...
def run(suite: Suite) -> None:
    ...
    def _():
        ...
        for seed in ("auto", "flighttest"):
            ...
            # **Enough in the tank to be about the flying.** Every one of
            # these runs is marginal on the 20 t a new hull carries — the
            # quote says `afford: False` for all three — and once orbits
            # gained a tilt the contact is out of the plane as well as across
            # it, which tipped one seed over: the computer flew it correctly
            # and arrived alongside still doing 25.9 m/s because the tank was
            # empty and nothing was left to brake with. That is a true fact
            # about the bill and `run_quote` is where it is claimed. This
            # check is about whether the computer can close and stop, so it
            # is given the mass to do it.
            conn.rcs = conn.opening_rcs = max(
                conn.rcs, free_sim.run_quote(game, conn, hull)["mass"] * 1.2)
            ticks = None
