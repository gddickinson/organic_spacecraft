...
from .chronicle_fights import _fight, _seek_trouble
...
def _refit_here(game, rng, plan) -> None:
    ...
    if not _put_in_at_a_yard(game):
        return
    # And bring the dead fittings back on line while alongside — free, and a
    # move any player makes. The driver never did, so an EMP that knocked
    # out the seed bay in year two stayed a fault for the decade: measured
    # on "chronicle-cover", ten years, thirteen fights, seed bay fitted and
    # `can_colonise` False the whole time, zero colonies.
    if game.ship.disabled:
        services_sim.clear_faults(game)
    ...
    for site in game.system.bodies:
        ...
        for klass in colonies_for(site.kind, game.research.unlocked):
            if colony_sim.can_found(game, game.system, site, klass.id)[0]:
                ...
                # Planting is the act the coverage check asks about, and
                # the roster at the end is not a record of it: measured on
                # "chronicle-cover", the driver planted over a spreading
                # decade and the Bloom ate every one — `len(game.colonies)`
                # read 0 and the check said the capability was never
                # exercised, which is the opposite of what happened.
                plan["planted"] = plan.get("planted", 0) + 1
                return
