...
# Standing off, split out at 487 lines; re-exported for the conn's step, the
# berthing knock and the panel, which all read it here.
from .sheer import (SHEER_FROM, SHEER_RATE, sheer_line,  # noqa: F401
                    sheer_step, sheers)
...
def _berth_names(game, contact) -> list:
    ...
    try:
        ...
        swallowed("control naming the berths", err)
        return []
...
def free(game, contact) -> list:
    ...
    out = [name for name in _berth_names(game, contact) if name not in taken]
    # Priority berthing, a Captain's perk (`sim/renown`): the first berth is
    # cleared of its trader rather than the captain being told to wait.
    if not out and taken and renown_sim.perk(game, "berth"):
        out = _berth_names(game, contact)[:1]
    return out
